...
elements.btnClear.addEventListener('click', () => {
    ...
    updateTerminal("<span class=\"prompt\">>_ </span>Prêt. En attente d'une opération...");
...
elements.btnCopy.addEventListener('click', () => {
    ...
    navigator.clipboard.writeText(elements.resultText.value).then(() => {
        ...
        elements.btnCopy.title = "Copié !";
        setTimeout(() => elements.btnCopy.title = originalTitle, 2000);
    });
});
...
function updateKeyPlaceholder() {
    ...
    if (algo === 'caesar') {
        ...
        elements.keyHint.textContent = "Décalage numérique (ex: 3). Laissez vide pour Force Brute (déchiffrement).";
    } else if (algo === 'vigenere') {
        elements.keyInput.placeholder = "Ex: SECRET";
        elements.keyHint.textContent = "Mot-clé alphabétique (ex: SECRET)";
    } else if (algo === 'hill') {
        elements.keyInput.placeholder = "Ex: 1,2; 3,4";
        elements.keyHint.textContent = "Matrice 2x2 ou 3x3 séparée par ; et ,";
    }
}
...
async function animateSteps(steps, finalResult) {
    setUIState(true);
    let currentHtml = "<span class=\"prompt\">>_ </span>Initialisation des calculs...\n\n";
    ...
    const delayPerChar = Math.max(5, Math.min(30, 2000 / (totalSteps * 20)));
    ...
    for (let i = 0; i < totalSteps; i++) {
        ...
        for (let j = 0; j < stepText.length; j++) {
            ...
            if (j % 5 === 0) updateTerminal(currentHtml); // Optimization: update DOM every 5 chars
    ...
    currentHtml += "\n<span class=\"prompt\">>_ </span>Opération terminée avec succès.";
    updateTerminal(currentHtml);
...
async function processCryptography(mode) {
    ...
    if (!text.trim()) {
        updateTerminal("<span style='color:#ff5f56'>Erreur : Le texte source est vide.</span>");
        return;
    }
    ...
    try {
        ...
        if (result.error) {
            updateTerminal(`<span style='color:#ff5f56'>Erreur : ${result.error}</span>`);
            return;
        }
        ...
        updateTerminal(`<span style='color:#ff5f56'>Erreur inattendue : ${e.message}</span>`);
    }
}
...
function processCaesar(text, keyStr, mode) {
    ...
    for (let i = 0; i < text.length; i++) {
        ...
        if (isLetter(char)) {
            ...
            final += char;
            // Ignore spaces/punctuation in steps to avoid noise
        }
    }
